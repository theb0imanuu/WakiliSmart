...
const DocumentRepository = () => {
  ...
  const handleDownload = (path) => {
    // This assumes the backend is serving files from the 'uploads' directory
  ...
  const handleRename = async (id, oldName) => {
    ...
    if (newName && newName !== oldName) {
      try {
        await api.patch(`/documents/${id}`, { name: newName });
        fetchDocuments();
      } catch (error) {
        alert('Failed to rename document: ' + (error.response?.data?.message || error.message));
      }
    }
  };
  ...
  const getFileIcon = (type) => {
    if (type.startsWith('image/')) {
      return <span className="material-symbols-outlined text-purple-500 text-[28px]">image</span>;
    }
    switch (type) {
      case 'application/pdf': return <span className="material-symbols-outlined text-red-500 text-[28px]">picture_as_pdf</span>;
      case 'application/msword':
      case 'application/vnd.openxmlformats-officedocument.wordprocessingml.document':
        return <span className="material-symbols-outlined text-blue-500 text-[28px]">description</span>;
      ...
      <div className="flex flex-col flex-1 bg-white dark:bg-[#111621] overflow-hidden">
              ...
              {user && user.role === 'ADVOCATE' && (
                <>
                  <button
                    ...
                    className="flex items-center justify-center h-10 px-4 bg-primary hover:bg-blue-700 text-white text-sm font-bold rounded-lg transition-colors shadow-sm cursor-pointer"
                  >
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-[#f8f9fc] dark:bg-[#0f131a]">
              ...
              {files.map((file) => (
                <div key={file.id} className="group relative flex items-center bg-white dark:bg-[#1a202c] border border-transparent hover:border-primary/30 rounded-lg p-3 shadow-sm hover:shadow-md transition-all cursor-pointer">
                  ...
                  <div className="grid grid-cols-12 gap-4 w-full items-center">
                    ...
                    <div className="col-span-3 hidden md:block text-sm text-slate-500">{new Date(file.createdAt).toLocaleDateString()}</div>
                    <div className="col-span-2 hidden md:block text-sm text-slate-500">{(file.size / 1024).toFixed(2)} KB</div>
                  ...
                  {user && user.role === 'ADVOCATE' && (
                    <div className="absolute right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 bg-white dark:bg-[#1a202c] shadow-sm rounded-md p-1 border border-slate-100 dark:border-slate-700">
                      <button onClick={() => handleDownload(file.path)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-500"><span className="material-symbols-outlined text-[18px]">download</span></button>
                      <button onClick={() => handleRename(file.id, file.name)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-500"><span className="material-symbols-outlined text-[18px]">edit</span></button>
                      <button onClick={() => handleDelete(file.id)} className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded text-slate-500"><span className="material-symbols-outlined text-[18px]">delete</span></button>
          ...
          {user && user.role === 'ADVOCATE' && (
            <div
              onClick={handleUploadClick}
              className="mt-8 flex flex-col items-center gap-4 rounded-xl border-2 border-dashed border-slate-300 dark:border-slate-700 bg-white dark:bg-[#1a202c] px-6 py-10 transition-colors hover:bg-slate-50 dark:hover:bg-[#232936] cursor-pointer"
            >
